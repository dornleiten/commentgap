---
title: "01_der-standard-scraping"
...
knitr::opts_chunk$set(
  cache.path = ".cache/knitr/01_der-standard-scraping/"
)
```
...
caps <- list(
  ...
  "moz:firefoxOptions" = list(
    args = list(
      "-user-agent=Flora Boewing, London School of Economics MSc Studentin, flora.boewing@gmail.com"
    ),
    prefs = list(
...
article_links
write.csv(article_links, "article_links.csv")

# remove liveberichte
article_links_clean <- article_links[which(startsWith(article_links, "/"))]
length(article_links_clean)
write.csv(article_links_clean, "article_links_cleaned.csv")
```

### merge 2 months together
```{r}
# import article links
article_links_clean_0922 <- unlist(c(read.csv("article_links_cleaned_0922.csv", skip=1, header=FALSE)[,2]))
article_links_clean_1022 <- unlist(c(read.csv("article_links_cleaned_1022.csv", skip=1, header=FALSE)[,2]))

article_links_clean <- c(article_links_clean_0922, article_links_clean_1022)
```


##Scrape article info
```{r}
# september article_info
...
```{r}
# import data
article_info_0922 <- read.csv("article_info_0922.csv")
article_info_0922 <- article_info_0922[4:11]
article_info_1022 <- read.csv("article_info_1022.csv")
article_info_1022 <- article_info_1022[2:9]

article_info <- rbind(article_info_0922, article_info_1022)

nrow(article_info)
View(article_info)
length(unique(article_info$title))
```
...
# 2. MANUALLY CLICK ONCE TO SHOW ALL REPLIES, any article discussion
...
```{r}
# import full dataset
comment_info_general <- read.csv("comment_info_091022_01.csv")

# delete some columns to be able to scrape, and some error rows
comment_info_general <- comment_info_general[-c(881581, 881582, 881583), 2:16]
#correct some errors
comment_info_general$pinned[c(881586, 881587, 881588)] <- " Angeheftet ·"

# correct another error
comment_info_general <- comment_info_general[-c(1037254, 1037257, 1037256, 1037255),]
comment_info_general$pinned[1037254] <- " Angeheftet ·"


nrow(comment_info_general)
View(comment_info_general[1:10,])
